...
import { deserializeLightclientOptimisticUpdate, deserializeSyncCommittee } from 'operator/common';
...
async function generate_data(b = 512) {
    const dirname = path.resolve();
    const rawData = fs.readFileSync(
        path.join(dirname, '../operator/data/4278368/optimisticUpdate.json')
    );
    const optimisticUpdate = deserializeLightclientOptimisticUpdate(JSON.parse(rawData.toString()));
    const syncCommitteeSerialized = fs.readFileSync(
        path.join(dirname, '../operator/data/4278368/syncCommittee.json')
    );
    const syncCommitte = deserializeSyncCommittee(JSON.parse(syncCommitteeSerialized.toString()));
    ...
    const pubkeys = syncCommitte.pubkeys.map((pubkey, idx: number) => {
        ...
        return [bigint_to_array(n, k, bigints[0]), bigint_to_array(n, k, bigints[1])];
    });

    const signature = toHexString(optimisticUpdate.syncAggregate.syncCommitteeSignature);
    ...
    console.log('Aggregate signature verified with computed aggregate public key');
    ...
    const zkpairingInput = {
        pubkey: ['0x' + aggPubKeyBigInts[0].toString(16), '0x' + aggPubKeyBigInts[1].toString(16)],
    ...
    const pubkeyAddrFilename = path.join(dirname, 'test', `input_pubkey_addr_${b}.json`);
    ...
    const verifyFilename = path.join(dirname, 'test', `input_aggregate_bls_verify_${b}.json`);
    ...
    const syncCommitteeSSZInput = {
        pubkeys: pubkeys,
        pubkeyHex: syncCommitte.pubkeys.map((pubkey) => hexToIntArray(toHexString(pubkey))),
        aggregatePubkeyHex: hexToIntArray(toHexString(syncCommitte.aggregatePubkey))
    };
    ...
    fs.writeFileSync(syncCommitteeFilename, JSON.stringify(syncCommitteeSSZInput));
    ...
    fs.writeFileSync(validSignedHeaderFilename, JSON.stringify(validSignedHeaderInput));
    ...
    console.log(
        'sum bits',
        validSignedHeaderInput.pubkeybits.reduce((partialSum: number, a) => partialSum + a, 0)
    );
}
